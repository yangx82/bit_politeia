...
class TestContextManager(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_compression_trigger(self):
        ...
        self.ctx_manager.summarizer_llm.ainvoke.return_value = MagicMock(content="Summary of huge messages")
        
        optimized, task_id, lineage = await self.ctx_manager.get_optimized_messages("sess_1", "next", history)
        
        # Should contain the summary checkpoint message
        has_checkpoint = any("[ITERATIVE CONTEXT SUMMARY]" in m.content for m in optimized if isinstance(m, (MagicMock, object)))
