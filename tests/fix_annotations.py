...
content = re.sub(
    r"(\s+)chroma_coordinator_host\s*=\s*", r"\1chroma_coordinator_host: str = ", content
)

# Fix 2: chroma_logservice_host
content = re.sub(
    r"(\s+)chroma_logservice_host\s*=\s*", r"\1chroma_logservice_host: str = ", content
)

# Fix 3: chroma_logservice_port
content = re.sub(
    r"(\s+)chroma_logservice_port\s*=\s*", r"\1chroma_logservice_port: int = ", content
)
