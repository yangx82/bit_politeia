...
class TestMembershipAdmissionLogic(unittest.IsolatedAsyncioTestCase):
    async def test_apply_and_approve_logic(self):
        ...
        with patch.object(p2p_mod, "p2p_service") as mock_p2p:
            ...
            with patch.object(
                bc_mod.bootstrap_client, "request_join", new_callable=AsyncMock
            ) as mock_request:
                ...
                mock_request.assert_called_once_with(
                    "group_A", "applicant_node_1", "I want to contribute."
                )

            with patch.object(
                bc_mod.bootstrap_client, "get_pending_joins", new_callable=AsyncMock
            ) as mock_list:
            ...
            with patch.object(
                bc_mod.bootstrap_client, "approve_join", new_callable=AsyncMock
            ) as mock_approve:
                ...
                mock_approve.assert_called_once_with(
                    "group_A", "applicant_node_1", "core_node_admin"
                )
