...
def pytest_configure(config):
    ...
    config.addinivalue_line(
        "markers", 
        "regression: 标记为回归测试，用于防止历史 Bug 复发"
    )
    config.addinivalue_line(
        "markers", 
        "critical: 标记为关键测试，失败时阻止部署"
    )
    config.addinivalue_line(
        "markers", 
        "property_test: 属性测试，使用随机输入验证"
    )
    config.addinivalue_line(
        "markers", 
        "performance: 性能测试"
...
def bug_metadata():
    ...
    return {
        'P2P_MSG_STATUS_2026_03_11': {
            ...
            'prohibited_phrases': [
                '已发送', '已完成', '已提交', '已处理', '已成功',
                'Message sent', 'File sent', 'Payment completed', 'Vote cast',
            ]
        }
    }
