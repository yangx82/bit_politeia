...
def test_get_skill_tools(skill_manager):
    ...
    assert "Extracts text from PDF files" in str(
        skill_manager.loaded_skills["pdf-reader"]["description"]
    )
...
def test_get_skill_index(skill_manager):
    ...
    assert (
        "- pdf-reader: Extracts text from PDF files for analysis. Use when user wants to read a PDF, summarize a document, or extract content from a research paper."
        in index
    )
