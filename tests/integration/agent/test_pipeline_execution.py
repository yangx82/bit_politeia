        ...
        context.tool_calls = [
            {
                ...
                'arguments': {
                    'recipient_id': '5faa8871...',
                    'content': 'Git-P2P 测试状态探查'
                }
            }
        ]
        context.tool_results = []
...
class TestPipelinePerformance:
    ...
    async def test_pipeline_execution_time(self):
        ...
        assert execution_time < 1.0, \
            f"Pipeline 执行时间过长: {execution_time}s"
    ...
    async def test_pipeline_memory_usage(self):
        ...
        # 内存增长应小于 10KB（模拟环境阈值）
        assert size_growth < 10240, \
            f"Pipeline 内存增长过大: {size_growth} bytes"
