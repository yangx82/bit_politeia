...
class TestPipelineStageOrdering:
    ...
    def test_no_duplicate_stages(self):
        ...
        assert len(stage_names) == len(unique_stages), f"Pipeline 存在重复阶段: {stage_names}"
                ...
                ctx.tool_calls = [
                    {
                        ...
                        "arguments": {"recipient": "node1", "content": "test"},
                    }
                ]
                return ctx
    ...
    def test_execute_to_notify_data_handoff(self):
        ...
        class MockNotifyStage:
            def execute(self, ctx):
                ...
                assert ctx.tool_results is not None, "NotifyStage 错误: tool_results 不存在"
