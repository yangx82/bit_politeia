...
def test_bootstrap_connection(bootstrap_url: str):
    ...
    try:
        ...
        if result == 0:
            ...
            print(f"  Suggestions:")
            print(f"    1. Check if bootstrap server is running")
            print(f"    2. Check firewall rules on server machine")
            print(f"    3. Verify server is bound to 0.0.0.0, not 127.0.0.1")
            return False
    except Exception as e:
    ...
    try:
        ...
        print(f"  Suggestion: Server may not be listening on the correct interface")
        return False
    except httpx.TimeoutException:
        print(f"✗ HTTP request timed out")
        print(f"  Suggestion: Network latency too high or server overloaded")
        return False
    except Exception as e:
    ...
    try:
        ...
        if resp.status_code == 200:
            ...
            print(f"✓ Topology endpoint accessible")
