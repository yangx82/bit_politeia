...
def test_proposal_lifecycle(governance_manager):
    ...
    # Valid votes
    governance_manager.receive_ballot(election.election_id, [Vote("v1", approval=True, reason="Good idea")])
    governance_manager.receive_ballot(election.election_id, [Vote("v2", approval=True, reason="Support")])
    governance_manager.receive_ballot(election.election_id, [Vote("v3", approval=True, reason="Why not")])
    
    # Rejections
    governance_manager.receive_ballot(election.election_id, [Vote("v4", approval=False, reason="Bad idea")])
    governance_manager.receive_ballot(election.election_id, [Vote("v5", approval=False, reason="Too costly")])
...
def test_proposal_failed_majority(governance_manager):
    ...
    # 1 Yes, 2 No
    governance_manager.receive_ballot(election.election_id, [Vote("v1", approval=True, reason="Yes")])
    governance_manager.receive_ballot(election.election_id, [Vote("v2", approval=False, reason="No")])
    governance_manager.receive_ballot(election.election_id, [Vote("v3", approval=False, reason="No")])
