...
def test_research_publication(governance_manager):
    ...
    proposal, election = governance_manager.initiate_research_publication(
        group_id="group_research", content="Quantum Consensus", pdf_hash="hash_123"
    )
...
def test_research_evaluation_quorum(governance_manager):
    ...
    success = governance_manager.receive_ballot(
        election.election_id, [Vote("author_node", reward_amount=100)]
    )
    assert success == False

    # 2. Others vote
    governance_manager.receive_ballot(
        election.election_id, [Vote("v1", reward_amount=100.0, reason="Good")]
    )
    governance_manager.receive_ballot(
        election.election_id, [Vote("v2", reward_amount=200.0, reason="Excellent")]
    )
    governance_manager.receive_ballot(
        election.election_id, [Vote("v3", reward_amount=100.0, reason="Okay")]
    )
    governance_manager.receive_ballot(
        election.election_id, [Vote("v4", reward_amount=0.0, reason="Bad")]
    )
    ...
    # Cast 5th vote
    governance_manager.receive_ballot(
        election.election_id, [Vote("v5", reward_amount=100.0, reason="Pass")]
    )
...
def test_negative_reward_validation(governance_manager):
    ...
    # Negative amount
    success = governance_manager.receive_ballot(
        election.election_id, [Vote("v1", reward_amount=-50, reason="Bad")]
    )
    assert success == False
...
def test_missing_reason_validation(governance_manager):
    ...
    # Missing reason
    success = governance_manager.receive_ballot(
        election.election_id, [Vote("v1", reward_amount=50, reason="")]
    )
    assert success == False
