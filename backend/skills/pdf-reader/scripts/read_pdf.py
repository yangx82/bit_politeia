    ...
    try:
        ...
        if output_file:
            ...
            with open(output_path, 'w', encoding='utf-8') as f:
                f.write(f"# PDF Chunked Reading Summary\n")
...
def read_pdf(file_path, start_page=None, end_page=None, mode='auto',
             chunk_size=DEFAULT_CHUNK_SIZE, output_file=None, info_only=False):
    """
    ...
    try:
        ...
        if mode == 'auto':
            if total_pages > AUTO_CHUNK_THRESHOLD:
                mode = 'chunked'
                print(f"⚠️ Auto-switching to chunked mode ({total_pages} pages > {AUTO_CHUNK_THRESHOLD} threshold)")
            else:
                mode = 'full'
        ...
        if len(text) > max_chars:
            text = text[:max_chars]
            print(f"\n⚠️ Content truncated to {max_chars} chars. Original: {len(text)} chars.", file=sys.stderr)
...
Examples:
  ...
  %(prog)s file.pdf --mode chunked --output temp/summary.md
    ...
    parser.add_argument("--mode", choices=['full', 'chunked', 'auto'], default='auto',
