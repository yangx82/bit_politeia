...
def get_paper(task_id: str):
    ...
    results = {"status": "success", "task_id": task_id, "files": []}

    for f in pdfs + markdowns:
        results["files"].append(
            {"name": f.name, "path": str(f), "type": "pdf" if f.suffix == ".pdf" else "markdown"}
        )

    if not results["files"]:
        return {
            "status": "error",
            "message": "No paper artifacts found yet. Task may still be running.",
        }
