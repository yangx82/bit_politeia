...
try:
    from backend.app.services.task_manager import task_manager, TaskStatus
except ImportError:
...
def start_research(topic: str):
    ...
    ARC_SRC = get_arc_src()
    if not ARC_SRC:
        return {"status": "error", "message": "AutoResearchClaw not found. Please run 'python scripts/setup_research_node.py' first."}
    ...
    venv_python = ARC_SRC / ".venv" / ("Scripts/python.exe" if sys.platform == "win32" else "bin/python")
    python_cmd = str(venv_python) if venv_python.exists() else sys.executable

    cmd = [
        python_cmd, "-m", "researchclaw", "run",
        "--topic", topic,
        "--auto-approve",
    ...
    try:
        ...
        return {"status": "error", "message": f"Failed to start research: {str(e)}"}
