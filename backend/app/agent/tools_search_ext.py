...
class CrossTaskSearchInput(BaseModel):
    query: str = Field(description="The scientific or transactional query to search for across all historical logs.")
    limit: int = Field(default=5, description="Maximum number of memory fragments to retrieve before summarization.")
...
class CrossTaskSearchTool(BaseTool):
    ...
    async def _arun(self, query: str, limit: int = 5) -> str:
        try:
            ...
            logger.info(f"CrossTaskSearch: Executing hybrid retrieval...")
