...
from ..p2p_community.bootstrap_client import GroupInfo, NodeRegistration, PeerAddress
...
class BootstrapService:
    ...
    def _initialize_root_group(self):
        ...
        logger.info(
            f"Bootstrap: Initialized with single Level 1 group: {first_group_id} ({self._groups[first_group_id].name})"
        )
    ...
    def get_topology_info(self, node_id: str | None = None) -> dict:
        """
        ...
        return {
            ...
            "hierarchy": {gid: list(members) for gid, members in self._group_members.items()},
            "pending": {
                gid: [r.to_dict() for r in requests]
                for gid, requests in self._pending_joins.items()
            },
            "stats": {
    ...
    def _create_child_group(self, parent_id: str) -> GroupInfo | None:
    ...
    def register_node(self, registration: NodeRegistration) -> bool:
        ...
        is_already_pending = any(
            r.node_id == registration.node_id for r in self._pending_joins[target_group_id]
        )
        if not is_already_pending:
            ...
            logger.info(
                f"Bootstrap: Node {registration.node_id} request to join Group {target_group_id} is PENDING approval."
            )
    ...
    def unregister_node(self, node_id: str) -> bool:
        ...
        if not target_nid:
            ...
            logger.warning(
                f"Service: Attempted to unregister unknown node {node_id}. Available IDs (prefix): {available}"
            )
            return False
        ...
        self._last_update = datetime.now()
        logger.info(
            f"Bootstrap: Node {node_id} has been manually unregistered and removed from topology."
        )
        return True

    # --- Tunnel Management ---

    def allocate_tunnel_port(self, node_id: str) -> int | None:
    ...
    def approve_node_join(self, group_id: str, node_id: str, approver_id: str) -> bool:
        ...
        if registration:
            ...
            if success:
                ...
                logger.info(
                    f"Bootstrap: Node {node_id} join to Group {group_id} APPROVED by {approver_id}"
                )
                return True
        return False
    ...
    def _perform_actual_join(self, target_group_id: str, registration: NodeRegistration) -> bool:
        ...
        if group.member_count == 1:
            if registration.node_id not in group.core_node_ids:
                ...
                logger.info(
                    f"Bootstrap: Node {registration.node_id} designated as Temporary Core Node for Group {target_group_id}"
                )

        # REVISED RULE 2: Election Triggers
        if group.member_count == 3:
            logger.info(
                f"ELECTION TRIGGER: Group {target_group_id} reached 3 members. Temporary Core should initiate formal election."
            )
        elif group.member_count == 11:
            logger.info(
                f"ELECTION TRIGGER: Group {target_group_id} reached 11 members. Triggering election for 2nd formal core node."
            )
        elif group.member_count == 19:
            logger.info(
                f"ELECTION TRIGGER: Group {target_group_id} reached 19 members. Triggering election for 3rd formal core node."
            )
        ...
        if group.member_count >= split_threshold:
            logger.info(
                f"Bootstrap: Group {target_group_id} reached capacity {group.member_count}. Triggering split."
            )
        ...
        self._last_update = datetime.now()
        logger.info(
            f"Bootstrap: Node {registration.node_id} joined L{group.level} group {target_group_id}"
        )
        return True
    ...
    def _split_group(self, group_id: str):
        ...
        logger.info(
            f"Bootstrap: L{old_group.level} Split (Alternating): {old_group.name} ({group_id}) -> {old_group.name} & {new_group.name} ({new_id})"
        )
        ...
        if not parent_id:
            ...
            logger.info(
                f"Bootstrap: Created new Parent L{parent_group.level} Group: {parent_id} ({parent_group.name})"
            )
    ...
    def _register_representative_to_parent(self, node_id: str, parent_id: str):
        ...
        if node_id not in self._group_members[parent_id]:
            ...
            logger.info(
                f"Bootstrap: Node {node_id} elected as rep to L{parent.level} Group {parent_id}"
            )

            # Recursive Split check
            split_threshold = community_config.rules.get("organization", {}).get(
                "split_threshold", 26
            )
