...
FRPS_PROCESS: subprocess.Popen | None = None
...
def start_frps():
    ...
    if not frps_path:
        logger.warning(
            "[Tunnel] frps binary not found. Tunnels will not be available. Put frps in backend/bin/"
        )
        return
...
@app.get("/topology")
async def get_topology(node_id: str | None = None) -> dict:
...
async def remove_node(node_id: str) -> dict[str, bool]:
    ...
    if not ALLOW_NODE_REMOVAL:
        logger.warning(
            "Node removal blocked: Safety toggle BOOTSTRAP_ALLOW_NODE_REMOVAL=true not set."
        )
        raise HTTPException(
        ...
        while True:
            ...
            try:
                ...
                if recipient_id:
                    ...
                    logger.info(
                        f"Relay Handler: Received {msg_type} from {node_id}. recipient={recipient_id} (IsGroup: {is_group})"
                    )

                    if is_group or (msg_type and str(msg_type).lower() == "group"):
                        # Group Broadcast (including Governance events like VOTE/PROPOSAL)
                        success = await relay_manager.broadcast_to_group(
                            node_id, recipient_id, message
                        )
                    else:
                    ...
                    logger.warning(
                        f"Relay: Received malformed message from {node_id} (No recipient_id or type)"
                    )
