...
class NetworkManager:
    ...
    async def initialize(self, node_id: str | None = None):
        ...
        if nat_manager.discover_gateway():
            if nat_manager.add_port_mapping(internal_port, internal_port, "TCP", "BitPoliteia P2P"):
                ...
                if public_ip:
                    logger.info(
                        f"[Network] UPnP NAT Traversal Successful. Public Endpoint: http://{public_ip}:{internal_port}"
                    )
        ...
        if always_tunnel or not nat_manager.public_ip:
            ...
            self.tunnel = TunnelClient(
                self.bootstrap.server_url, self.local_node_id or "default", internal_port
            )
    ...
    def _sync_topology(self, topology_data: dict):
        ...
        stale_nodes = [
            nid for nid in self.nodes if nid not in server_node_ids and nid != self.local_node_id
        ]
    ...
    async def register_node(self, node: Node):
        ...
        if nat_manager.public_ip:
            ...
            logger.info(
                f"[Network] Using discovered public endpoint for registration: {host}:{port}"
            )
    ...
    async def handle_relayed_message(self, message_data: dict):
        ...
        try:
            ...
            if "timestamp" not in message_data:
                logger.warning(
                    f"[Network] Received relayed message missing 'timestamp'. Content: {str(message_data)[:200]}"
                )
    ...
    async def broadcast_group_config(self, group_id: str, core_node_ids: list[str]):
        ...
        logger.info(
            f"[Network] Broadcasting Group Config Update for {group_id} (Core Nodes: {len(core_node_ids)})"
        )
    ...
    async def route_message(
        self, message: SignedMessage, from_relay: bool = False, gossip_forward: bool = True
    ) -> bool:
            ...
            if node_id in self.nodes:
                ...
                if not success:
                    ...
                    if from_relay:
                        self._log_throttled(
                            "warning",
                            f"[Network] Dropping message to {node_id} to prevent Relay Loop.",
                        )
                        return False
        ...
        if is_group_broadcast_type or is_group_id:
            ...
            if group_id in self.groups:
                ...
                targets = [
                    m_id
                    for m_id in members
                    if m_id != self.local_node_id and m_id != message.sender_id
                ]
                ...
                logger.info(
                    f"[Network] Broadcast for {message.message_type.value}: Initiating parallel direct delivery to {len(targets)} members."
                )
                ...
                if not failed_ids:
                    logger.info(
                        f"[Network] Broadcast for {message.message_type.value}: All {len(targets)} members reached via direct HTTP."
                    )
                    return True
                ...
                logger.info(
                    f"[Network] Broadcast for {message.message_type.value}: {len(failed_ids)} members unreachable directly. Requesting Partial Relay Broadcast."
                )
                ...
                if not from_relay:
                    logger.info(
                        f"[Network] Group {group_id} not found locally. Pushing to Relay for full broadcast."
                    )
    ...
    async def _gossip_broadcast(
        self, message: SignedMessage, group_id: str, exclude_sender: bool = True
    ):
        """
        ...
        logger.info(
            f"[Gossip] Forwarding {message.message_type.value} message {message.message_id[:8]}... to {len(targets)} group members"
        )
        ...
        logger.info(
            f"[Gossip] Forwarded to {success_count}/{len(targets)} peers for message {message.message_id[:8]}..."
        )
    ...
    def get_network_structure(self):
        ...
        return {
            ...
            "nodes": {
                n_id: {
                    "name": n.name,
                    "public_key": n.public_key[:16] + "...",
                    "is_online": n.is_online,
                }
                for n_id, n in self.nodes.items()
            },
            "relay_connected": getattr(self, "relay_client", None)
            and self.relay_client.websocket is not None,
        }
