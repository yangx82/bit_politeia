...
def test_logic():
    ...
    # Clean up
    if inbox_path.exists(): os.remove(inbox_path)
    if proc_path.exists(): os.remove(proc_path)
    ...
    if inbox_path.exists():
        if proc_path.exists():
            with open(proc_path, 'a') as pf:
                with open(inbox_path, 'r') as ifile:
                    pf.write(ifile.read())
            os.remove(inbox_path)
        else:
            os.rename(inbox_path, proc_path)
    
    print(f"After Rename - Inbox exists: {inbox_path.exists()}, Processing exists: {proc_path.exists()}")
