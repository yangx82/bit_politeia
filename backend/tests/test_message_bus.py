...
async def test_concurrent_dispatch():
    ...
    if "fast_msg1" in results and "slow_msg1" not in results:
        logger.info("SUCCESS: Fast subscriber received message while slow one is still sleeping (Concurrent Dispatch Works!)")
    else:
        logger.error("FAILURE: Order was not as expected or fast subscriber was blocked. Results: %s", results)
