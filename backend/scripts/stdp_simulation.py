...
def simulate_stdp_curve():
    ...
    # 计算权重变化
    delta_w_values = [
        stdp_weight_change(dt, A_plus, A_minus, tau_plus, tau_minus) for dt in delta_t_range
    ]
...
def compare_time_constants():
    ...
    for tau in tau_values:
        delta_w_values = [stdp_weight_change(dt, 0.005, 0.005, tau, tau) for dt in delta_t_range]
...
def generate_simulation_results(output_dir="docs/technical"):
    ...
    for dt in key_points:
        dw = stdp_weight_change(
            dt, params["A_plus"], params["A_minus"], params["tau_plus"], params["tau_minus"]
        )
        ax1.plot(dt, dw, "ro", markersize=6)
        ax1.annotate(
            f"Δt={dt}ms\nΔW={dw:.6f}",
            xy=(dt, dw),
            xytext=(dt + 5, dw + 0.001),
            fontsize=8,
            ha="left",
    ...
    for tau, color in zip([10.0, 20.0, 50.0], colors):
        ax2.plot(delta_t_range, tau_results[tau], linewidth=2, label=f"τ = {tau}ms", color=color)
    ...
    for dt in [-50, -20, -10, 10, 20, 50]:
        ...
        key_results.append({"delta_t": dt, "delta_w": dw, "effect": effect})
