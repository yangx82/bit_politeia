...
def generate_self_signed_cert(cert_path="keys/server.crt", key_path="keys/server.key", days=365):
    ...
    subject = x509.Name(
        [
            ...
            x509.NameAttribute(NameOID.ORGANIZATION_NAME, "Bit Politeia"),
    ...
    try:
        ...
        s.close()
        alt_names.append(
            x509.IPAddress(type("IPv4Address", (object,), {"compressed": local_ip}))
        )  # Pseudo-code if ipaddress module used, but x509 expects specific types.
